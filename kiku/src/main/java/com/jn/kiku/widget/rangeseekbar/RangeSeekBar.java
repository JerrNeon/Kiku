package com.jn.kiku.widget.rangeseekbar;
...
import com.jn.kiku.utils.DensityUtils;
...
public class RangeSeekBar<T extends Number> extends AppCompatImageView {
    ...
    private static final int DEFAULT_TEXT_SIZE_IN_DP = 12;//顶部的字体大小
    private static final int DEFAULT_TEXT_DISTANCE_TO_BUTTON_IN_DP = 4;//离底部的距离
    private static final int DEFAULT_TEXT_DISTANCE_TO_TOP_IN_DP = 4;//离顶部的距离
    ...
    private OnRangeRateSeekBarChangeListener mRateListener;
    ...
    private void init(Context context, AttributeSet attrs) {
        float barHeight;
        int thumbNormal = R.drawable.ic_kiku_seekbar;
        int thumbPressed = R.drawable.ic_kiku_seekbar;
        int thumbDisabled = R.drawable.ic_kiku_seekbar;
        int thumbShadowColor;
        int defaultShadowColor = Color.argb(75, 0, 0, 0);
        int defaultShadowYOffset = DensityUtils.dp2px(context, 2);
        int defaultShadowXOffset = DensityUtils.dp2px(context, 0);
        int defaultShadowBlur = DensityUtils.dp2px(context, 2);

        if (attrs == null) {
            setRangeToDefaultValues();
            internalPad = DensityUtils.dp2px(context, INITIAL_PADDING_IN_DP);
            barHeight = DensityUtils.dp2px(context, LINE_HEIGHT_IN_DP);
        ...
        textSize = DensityUtils.sp2px(context, DEFAULT_TEXT_SIZE_IN_DP);
        distanceToTop = DensityUtils.dp2px(context, DEFAULT_TEXT_DISTANCE_TO_TOP_IN_DP);
        textOffset = !showTextAboveThumbs ? 0 : this.textSize + DensityUtils.dp2px(context,
            ...
            case MotionEvent.ACTION_MOVE:
                if (pressedThumb != null) {
                    ...
                    if (notifyWhileDragging && mRateListener != null) {
                        double minValue = DensityUtils.calculateRate(valueToString(getSelectedMinValue()));
                        double maxValue = DensityUtils.calculateRate(valueToString(getSelectedMaxValue()));
    ...
    protected synchronized void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int height = thumbImage.getHeight()
                + (!showTextAboveThumbs ? 0 : DensityUtils.dp2px(getContext(), HEIGHT_IN_DP))
    ...
    protected synchronized void onDraw(@NonNull Canvas canvas) {
        ...
        if (showTextAboveThumbs && (activateOnDefaultValues || !selectedValuesAreDefault)) {
            ...
            if (!singleThumb) {
                ...
                int spacing = DensityUtils.dp2px(getContext(), TEXT_LATERAL_PADDING_IN_DP);
    ...
    public void setOnRangeRateSeekBarChangeListener(OnRangeRateSeekBarChangeListener listener) {
        this.mRateListener = listener;
    }

    public interface OnRangeRateSeekBarChangeListener {
        void onRangeSeekBarValuesChanged(RangeSeekBar bar, long minValue, long maxValue);
    }
}
