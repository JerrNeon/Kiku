 ...
 * @Description: (百度地图定位回调)
 * @create by: chenwei
 * @date 2018/6/12 17:40
 */
public class BDLocationListenerIml implements BDLocationListener, ILogToastView {
    ...
    private void locationSuccess(BDLocation location) {
        if (mLocationResultListener != null)
            mLocationResultListener.onSuccess(location);
    }
     ...
     * @param location BDLocation
     */
    private void locationFailure(BDLocation location) {
        if (mLocationResultListener != null)
            mLocationResultListener.onFailure(location);
    }
